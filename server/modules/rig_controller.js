...
function rigControl(startRig) {
    ...
    ssh.on("ready", () => {
        ...
        ssh.sftp((er, sftp) => {
            ...
            sftp.fastPut(FILE_PATHS.CONFIG_FILE_PATH, RIG_CONFIG.PATH, (err) => {
                if (err) {
...
function executeCommand(ssh) {
    ssh.exec(RIG_CONFIG.APP_RUNNING_COMMAND, (err, stream) => {
        if (err) {
...
function runTheRecordingApp(ssh, startRig) {
    ...
    ssh.exec(RIG_CONFIG.KILL_PYTHON_APPS_COMMAND, (err, stream) => {
